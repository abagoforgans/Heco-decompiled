...
function getConfirmations(uint256 arg1) {
    require calldata.size - 4 >= 32
    if owners.length:
        mem[128 len 32 * owners.length] = code.data[9168 len 32 * owners.length]
    idx = 0
    s = 0
    while idx < owners.length:
        mem[0] = address(owners[idx].field_0)
    ...
    if s:
        mem[(32 * owners.length) + 160 len 32 * s] = code.data[9168 len 32 * s]
    t = 0
    while t < s:
...
function getTransactionIds(uint256 arg1, uint256 arg2, bool arg3, bool arg4) {
    ...
    if transactionCount:
        mem[128 len 32 * transactionCount] = code.data[9168 len 32 * transactionCount]
    idx = 0
    s = 0
    while idx < transactionCount:
        if not arg3:
    ...
    if arg2 - arg1:
        mem[(32 * transactionCount) + 160 len 32 * arg2 - arg1] = code.data[9168 len 32 * arg2 - arg1]
    idx = arg1
    while idx < arg2:
